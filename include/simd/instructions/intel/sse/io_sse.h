...
static apply(
   ...
   typename vector_constants_t< sse < uint8_t > > ::array_t
   tmp;
   _mm_store_si128( reinterpret_cast< __m128i * >( tmp.data( ) ), vindex );
   for(
      std::size_t i = 0; i < vector_constants_t < sse < uint8_t > > ::vector_element_count_t::value; ++i
      ) {
...
static apply(
   ...
   typename vector_constants_t< sse < uint16_t > > ::array_t
   tmp;
   _mm_store_si128( reinterpret_cast< __m128i * >( tmp.data( ) ), vindex );
   for(
      std::size_t i = 0; i < vector_constants_t < sse < uint16_t > > ::vector_element_count_t::value; ++i
      ) {
      tmp[ i ] = *( base + ( tmp[ i ] * Scale ) );
   }
   return _mm_load_si128( reinterpret_cast< __m128i const * >( tmp.data( ) ) );
