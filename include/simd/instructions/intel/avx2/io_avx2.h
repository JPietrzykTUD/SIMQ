...
*/
   ...
   struct gather_impl< avx2< uint8_t >, Scale > {
      ...
      static apply(
         ...
         typename vector_constants_t< avx2< uint8_t > >::array_t tmp;
         _mm256_store_si256( reinterpret_cast< __m256i * >( tmp.data() ), vindex );
         for( std::size_t i = 0; i < vector_constants_t< avx2< uint8_t > >::vector_element_count_t::value; ++i ) {
   ...
   struct gather_impl< avx2< uint16_t >, Scale > {
      ...
      static apply(
         ...
         typename vector_constants_t< avx2< uint16_t > >::array_t tmp;
         _mm256_store_si256( reinterpret_cast< __m256i * >( tmp.data() ), vindex );
         for( std::size_t i = 0; i < vector_constants_t< avx2< uint16_t > >::vector_element_count_t::value; ++i ) {
            tmp[ i ] = *( base + ( tmp[ i ] * Scale ) );
         }
         return _mm256_load_si256( reinterpret_cast< __m256i const * >( tmp.data() ) );
      }
