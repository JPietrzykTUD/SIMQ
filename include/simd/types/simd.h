...
namespace tuddbs {
   ...
   template <typename T, std::size_t N, std::size_t Alignment> struct alignas(Alignment) aligned_array : public std::array<T,N> {};
   ...
   struct vector_constants_t {
      ...
      using number_of_vectors_per_mask_t =
         std::integral_constant< std::size_t, ( sizeof( typename VectorExtension::mask_t ) * 8 ) / vector_element_count_t::value >;
