...
namespace tuddbs {
   class cpu_freq_monitor :
      public singleton< cpu_freq_monitor > {
      public:
         ...
         void frequency_monitoring( ) {
            ...
            while( !halt ) {
               ...
               if( tick >= last_expected_ticks ) {
                  ...
                  for(
                     size_t i = 0; i < last_core_count; ++i
                     ) {
                     freq_array[ i ] =
                        ( uint64_t * ) realloc( freq_array[ i ], sizeof( uint64_t ) * last_expected_ticks );
                  }
               }
      ...
      public:
         ...
         std::string get_data( ) const {
            ...
            for(
               ...
               ss << "Core" << i << "@";
               for(
                  size_t j = 0; j < registered_ticks; ++j
                  ) {
                  ss << freq_array[ i ][ j ];
