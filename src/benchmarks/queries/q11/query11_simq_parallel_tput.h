   ...
   struct simq_wl_q11_parallel_tput {
      ...
      static void run(
         ...
         experiment_query_11_12< VectorExtension, ColumnCount, QueryCount, BatchSize >::print_experiment_result(
            0, datagenerator, "SIMQ-PAR-Budget", "BITMASK", Strategy< column_array_t, QueryCount,
      ...
      static void run(
         ...
         auto
            magic                      =
            [ &ready_future, &filter_result_masks, &aggregation_result_cols, &global_query_counter,
              &datagenerator, &finished ](
               const size_t tid
            ) {
               ready_future.wait( );
               ...
               typename VectorExtension::mask_t * filter_result_bitmask_ptr;
               typename VectorExtension::mask_t * filter_result_bitmask_ptr_new;
               T                                * aggregation_result_column_ptr;
               
               size_t pos = 0;
               while( !finished ) {
                  filter_result_bitmask_ptr =
                     ( typename VectorExtension::mask_t * ) filter_result_masks[ tid ]->data_ptr;
                  aggregation_result_column_ptr = aggregation_result_cols[ tid ]->data_ptr;
                  pos                                              = 0;
                  for(
                     std::size_t                     query_id      = 0; query_id < QueryCount; ++query_id
                     ) {
                  ...
                  while( svb_filter.has_next() ) {
                     ...
                     aggregate_impl< VectorExtension, svb_t, aggregate_mask_add
                     >::apply(
