...
void run_sequential( std::size_t data_size, bool use_mcdram ) {
   ...
   for(
      std::size_t                              i = 0; i < ColumnCount; ++i
      ) {
      auto column = create_column( T, data_count, use_mcdram );
      T * data = column->data_ptr;
      for(
         std::size_t j = 0; j < data_count; ++j
         ) {
         data[ j ] = dist( engine );
      }
   ...
   for(
      ...
      for(
         ...
         for(
            std::size_t query_id = 0; query_id < ( QueryCount / ColumnCount ); ++query_id
            ) {
            ...
            for(
               std::size_t i = 0; i < data_count_vec; ++i
               ) {
...
>
void run_single_build_variant( std::size_t data_size, bool use_mcdram ) {
   using namespace tuddbs;
   using T = typename VectorExtension::base_t;
   std::size_t                       data_count  = data_size / sizeof( T );
   typename rng< T >::engine_t       engine( 0xC004C0FEEBADC0DE );
   ...
   for(
      ...
      typename VectorExtension::vector_t                      result      = set_zero< VectorExtension >( );
      for(
         std::size_t                                          i           = 0; i < data_count; i += incrementor
         ) {
         result = bitwise_xor< VectorExtension >( result, svb.get_and_increment( ) );
      }
      typename vector_constants_t< VectorExtension >::array_t buff;
      [[maybe_unused]] T * out_tmp = store< VectorExtension >( buff.data( ), result );
      std::size_t    lanes_per_query =
                        vector_constants_t< VectorExtension >::vector_element_count_t::value / QueryCount;
      for(
         std::size_t i               = 0; i < QueryCount; ++i
         ) {
         T              result_of_query = 0;
         for(
            std::size_t j               = 0; j < lanes_per_query; ++j
            ) {
...
void run_vector_extension( std::size_t data_size ) {
   using namespace tuddbs;
   run_sequential< VectorExtension, ColumnCount, QueryCount >( data_size, true );
   run_sequential< VectorExtension, ColumnCount, QueryCount >( data_size, false );
   //mask broadcast is not supported from xeon phi knl
   run_single_build_variant< simq_vector_builder_buffer_t, VectorExtension, ColumnCount, QueryCount >(
      data_size, true
   );
   run_single_build_variant< simq_vector_builder_buffer_t, VectorExtension, ColumnCount, QueryCount >(
      data_size, false
   );
   run_single_build_variant< simq_vector_builder_set_t, VectorExtension, ColumnCount, QueryCount >( data_size, true );
   run_single_build_variant< simq_vector_builder_set_t, VectorExtension, ColumnCount, QueryCount >( data_size, false );
   if( sizeof( typename VectorExtension::base_t ) > 2 ) {
      run_single_build_variant< simq_vector_builder_gather_t, VectorExtension, ColumnCount, QueryCount >(
         data_size, true
      );
      run_single_build_variant< simq_vector_builder_gather_t, VectorExtension, ColumnCount, QueryCount >(
         data_size, false
...
int main( void ) {
   using namespace tuddbs;
   const char * description = "#Results of the microbenchmark for GATHER and SEQUENTIAL LOAD on KNL.";
   
   std::cout << description << "\n" << get_definitions( "#" ) <<
             "id;data_count;data_size;vector_extension_name;data_type_size_bit;" <<
             "build_variant_name;column_count;query_count;use_mcdram;time_needed_ns;result;dummy_val\n";
