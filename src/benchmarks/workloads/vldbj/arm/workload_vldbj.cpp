...
template<
   class VectorExtension, std::size_t ColumnCount, std::size_t QueryCount, std::size_t NumberOfSucceedingOperators, std::size_t BatchSize = 0
>
struct workload {
   template<
      std::size_t SO = NumberOfSucceedingOperators, typename std::enable_if< SO == 1, std::nullptr_t >::type = nullptr
   >
   static void run(
      ...
      simq_wl_q11< simq_vector_builder_set_t, VectorExtension, ColumnCount, QueryCount, BatchSize >::run(
         datagenerator
      );
...
//      }
   ...
   template<
      std::size_t SO = NumberOfSucceedingOperators, typename std::enable_if< SO == 2, std::nullptr_t >::type = nullptr
   >
   static void run(
      std::size_t data_size
   ) {
      ...
      std::cerr
         ...
         << ".\n";
      std::cerr << "      Single Threaded... ";
      seq_wl_q12_two_Stage2_ops< neon< uint16_t >, 4, 4 > ::run(
         datagenerator
      );
      ...
      std::cerr << "Done.\n       Multi Threaded...Two";
      seq_parallel_wl_q12_two_Stage2_ops< neon< uint16_t >, 4, 4 > ::run( datagenerator );
      seq_parallel_wl_q12_two_Stage2_ops_tput< neon< uint16_t >, 4, 4 > ::run( datagenerator );
   ...
   template<
      std::size_t SO = NumberOfSucceedingOperators, typename std::enable_if< SO == 3, std::nullptr_t >::type = nullptr
   >
   static void run(
      std::size_t data_size
   ) {
      ...
      std::cerr
         ...
         << ".\n";
      std::cerr << "      Single Threaded... ";
      seq_wl_q12_three_Stage2_ops< neon< uint16_t >, 4, 2, 2 > ::run(
         datagenerator
      );
      ...
      std::cerr << "Done.\n       Multi Threaded... ";
      seq_parallel_wl_q12_three_Stage2_ops< neon< uint16_t >, 4, 2, 2 > ::run( datagenerator );
      seq_parallel_wl_q12_three_Stage2_ops_tput< neon< uint16_t >, 4, 2, 2 > ::run( datagenerator );
...
void run_experiment( std::size_t data_size ) {
   ...
   workload< neon < uint16_t >, 1, 1, 1 > ::run( data_size );
   workload< neon < uint16_t >, 1, 2, 1 > ::run( data_size );
   workload< neon < uint16_t >, 1, 4, 1 > ::run( data_size );
   std::cerr << "Experiment column count\n";
   workload< neon < uint16_t >, 2, 8, 1 > ::run( data_size );
   workload< neon < uint16_t >, 4, 8, 1 > ::run( data_size );
   workload< neon < uint16_t >, 8, 8, 1 > ::run( data_size );
   std::cerr << "Experiment succeeding operators\n";
   workload< neon < uint16_t >, 1, 8, 2 > ::run( data_size );
   workload< neon < uint16_t >, 1, 8, 3 > ::run( data_size );
...
int main( void ) {
   ...
   auto str = oss.str( );
   std::cerr << "Executing Benchmarks for VLDB on ARM.\n";
   std::cout << str.c_str( ) << "\n";
