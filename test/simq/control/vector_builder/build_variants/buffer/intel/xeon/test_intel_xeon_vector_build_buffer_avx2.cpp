...
#define MAX_VECTOR_ELEMENT_COUNT 32
#define ELEMENT_COUNT 1024

template< typename T >
void print( __m256i a ) {
   std::array< T, 32/sizeof(T) > tmp;
   _mm256_store_si256( reinterpret_cast< __m256i * >( tmp.data() ), a );
   for( std::size_t i = 0; i < 32/sizeof(T); ++i ) {
      std::cout << std::setw( 3 ) << (int)tmp[ 32/sizeof(T) - 1 -i ] << " ";
   }
   std::cout << "\n";
}
...
bool test( void ) {
   ...
   column_array< avx2< T >, ColumnCount > test_column_array;
   ...
   simq_vector_builder_t<
      ...
      column_array< avx2< T >, ColumnCount >, QueryCount > a( test_column_array );

   T * const output_buffer_orig = ( T * ) acquire( data_count * sizeof( T ) * QueryCount, 32 );
   T * const should_be_buffer = ( T * ) acquire( data_count * sizeof( T ) * QueryCount, 32 );
   T * output_buffer = output_buffer_orig;
   std::size_t incrementor = a.get_incrementor();
   for( std::size_t i = 0; i < data_count; i+=incrementor ) {
      __m256i tmp_vec = a.get_and_increment();
      _mm256_store_si256( reinterpret_cast< __m256i * >( output_buffer), tmp_vec );
      output_buffer += ( 32 / sizeof( T ) );
   }


   std::size_t element_count = 32 / sizeof( T );
