...
bool test( std::size_t data_count ) {
   ...
   using predicate_array_t = value_array< avx2< T >, QueryCount >;
...
//   print< avx2 < T > >( std::cout, predicate_vec, "Pred" );
   ...
   filter_impl< avx2< T >, svb_t, point_filter_lt_core >::apply( bitmask_column, svb_for_filter, predicate_vec );

//   for( std::size_t i = 0; i < bitmask_column->data_count; ++i ) {
//      std::cout << "[" << std::setw( 4 ) << i << "]: " << std::bitset< sizeof( T ) * 8 >{ bitmask_column->data_ptr[ i ] } << "\n";
//   }

   aggregate_impl< avx2< T >, svb_t, aggregate_mask_add >::apply( result_column, svb_for_aggregation, bitmask_column );
   [[maybe_unused]]typename avx2< T >::base_t * tmp = store< avx2< T > >(
      result_column->data_ptr,
      aggregate_mask_add< avx2< T > >::template finalize< svb_t::lanes_per_query_t::value >(
         load< avx2< T > >( result_column->data_ptr )
      )
   );
