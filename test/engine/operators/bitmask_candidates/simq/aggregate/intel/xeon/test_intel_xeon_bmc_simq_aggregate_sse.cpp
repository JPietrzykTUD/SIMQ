...
bool test( std::size_t data_count ) {
    ...
    *
    */
   using column_array_t = column_array< sse< T >, ColumnCount >;
...
//   print< sse < T > >( std::cout, predicate_vec, "Pred" );
   ...
   filter_impl< sse< T >, svb_t, point_filter_lt_core >::apply( bitmask_column, svb_for_filter, predicate_vec );

//   for( std::size_t i = 0; i < bitmask_column->data_count; ++i ) {
//      std::cout << "[" << std::setw( 4 ) << i << "]: " << std::bitset< sizeof( T ) * 8 >{ bitmask_column->data_ptr[ i ] } << "\n";
//   }

   aggregate_impl< sse< T >, svb_t, aggregate_mask_add >::apply( result_column, svb_for_aggregation, bitmask_column );

   [[maybe_unused]]typename sse< T >::base_t * tmp = store< sse< T > >(
      result_column->data_ptr,
      aggregate_mask_add< sse< T > >::template finalize< svb_t::lanes_per_query_t::value >(
         load< sse< T > >( result_column->data_ptr )
      )
   );

   bool result_val = true;
   for( std::size_t i = 0; i < QueryCount; ++i ) {
      if( result_column->data_ptr[ i ] != ( (i+1) * 2 * 2 ) ) {
         std::cerr << "ValueSize: " << sizeof( T ) * 8 << ". [ColumnCount, QueryCount] == [" << ColumnCount << ", " << QueryCount << "]. ";
         std::cerr << "Query #" << i << ". ";
         std::cerr << "Error.";
         std::cerr << "Expected: " << ( (i+1) * 2 * 2 ) << ". Is: " << ( uint64_t ) result_column->data_ptr[ i ] << "\n";
         result_val = false;
      }
   }
