...
bool test( std::size_t data_count ) {
   ...
   using value_array_t = value_array< sse< T >, QueryCount >;
   ...
   for( std::size_t i = 0; i < data_count; i += incrementor ) {
      sequential_buffer = store< sse< T > >( sequential_buffer, svb_for_gather.get_and_increment( ) );
   }
   sequential_filter_impl< sse< T >, point_filter_lt_core >::apply( expected_column, sequential_column, predicate_vec );
   /**
   ...
   filter_impl< sse< T >, svb_t, point_filter_lt_core >::apply( result_column, svb, predicate_vec );
